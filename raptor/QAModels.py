...
# class GROQQAModel(BaseQAModel):
#     def __init__(self, model="gpt-3.5-small"):
#         ''''
#         Initializes the GPT-3 model with the specified model version.
...
class GPT4QAModel(BaseQAModel):
    ...
    def _attempt_answer_question(
        ...
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": f"You are Question Answering Portal. Integrate the {context} into the answer to the user {question}. Do not include the user id in the response. Instead substitute You for the  user id in your response"},
                {
                    "role": "user",
                    "content": f"Given Context: Give the best full answer amongst the option to question {question}",
                },
            ],
            stream=True,
            temperature=0,
...
class GROQAAModel(BaseQAModel):
    ...
    def _attempt_answer_question(
        ...
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": f"You are Question Answering Portal. Integrate the {context} into the answer to the user {question}"},
                {
                    "role": "user",
                    "content": f"Given Context: Add {context} to the beginning of your response. Give the best full answer amongst the option to question {question}",
                },
            ],
            stream=True,
            temperature=0,
